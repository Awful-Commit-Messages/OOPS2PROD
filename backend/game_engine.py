...
class OrganicMultiAgentEngine:
    ...
    async def process_moment(self, player_input: Optional[str] = None) -> dict:
        ...
        self.state.tension_level = max(
            1, min(10, self.state.tension_level + tension_delta)
        )
        ...
        if affected_npc_ids:
            logger.info(
                f"Phase 2: Querying {len(affected_npc_ids)} NPCs in parallel..."
            )
        ...
        logger.info(
            "Phase 4: Narrator is retelling the story through a distorted lens..."
        )
